...
def post(text):
    ...
    # example text='“A merry little surge of electricity piped by automatic alarm from the mood organ beside his bed awakened Rick Deckard.”'
    type.click()
...
#Here's an example of our post function:
#post('"You will rejoice to hear that no disaster has accompanied the commencement of an enterprise which you have regarded with such evil forebodings."')

#Let's see what else we can do?


def text_grab():
    # This just gets us back to the main page as above
    driver = webdriver.Chrome(chrome_options=chrome_options,executable_path=DRIVER_PATH)
    driver.get(url)
    username = driver.find_element('id', 'loginUsername').send_keys(config.username)
    password= driver.find_element('id', 'loginPassword').send_keys(config.password)
    ...
    # Now by inspecting the page, you can see all the posts are held in a class,
    # let's see what we can do with that.
    
    time.sleep(1.5)
    posts = driver.find_elements(By.CSS_SELECTOR, "div.rpBJOHq2PR60pnwJlUyP0")
   
    divs = posts[0].find_elements(By.CSS_SELECTOR, "div")
    #imgs = posts.find_elements_by_css_selector('img')

    #These are examples if you want to grab different stuff inside this parent class:
    #classes = [item.get_attribute("class") for item in divs]
    #srcs = [item.get_attribute("src") for item in imgs]

    # Now we've got all the classes inside of posts (there's quite a lot) but makes it easier to look at
    # We've got the issue now that as we go down the page we'll load in more of these posts so we should
    # set a limit

    # All our posts are of the class: _1oQyIsiPHYt6nx7VOmd1sz so...
    text_list=[]
   
    wait.until(EC.presence_of_element_located((By.CSS_SELECTOR, "h3._eYtD2XCVieq6emjKBH3m")))
    # This scrolls us down so we can load some more in

    driver.execute_script("window.scrollTo(0,document.body.scrollHeight)")

    # Find our posts
    post_texts = driver.find_elements(By.CSS_SELECTOR, "h3._eYtD2XCVieq6emjKBH3m")

    for v in range(len(post_texts)):
        # This gets the text from the invididual post
        text=post_texts[v].text

        # We've got an issue with empty texts, I'm guessing they haven't loaded in properly...
        if text!= '':
...
#This is all we need to grab the text of the main page of reddit, good to see what posts are being sent your way
text_grab()